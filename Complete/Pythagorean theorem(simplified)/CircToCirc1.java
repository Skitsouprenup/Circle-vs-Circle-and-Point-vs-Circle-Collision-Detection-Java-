...
public class CircToCirc1 {
	...
	public CircToCirc1() {
		...
		EventQueue.invokeLater(new Runnable(){
			...
			@Override
			public void run() {
				JFrame jf = new JFrame("CircToCirc1");
				Panel pnl = new Panel();
	...
	void updateData(){
		...
		//Offset to move the circle's center to the mouse pointer
		c1OffsetX = c1.getX() - c1.getWidth() * 0.5f;
		c1OffsetY = c1.getY() - c1.getHeight() * 0.5f;
		//get the a^2 + b^2 in the formula: a^2 + b^2 = c^2
		float xSide = c1.getX() - c2CenterX;
		float ySide = c1.getY() - c2CenterY;
		xSide = xSide * xSide;
		ySide = ySide * ySide;
		//distance(hypotenuse) of two circles' centers
		//simplify the formula: a^2 + b^2 = c^2 -> c = sqrt(a + b)
		float distance = (float) Math.sqrt(xSide + ySide);
		
		if(distance < totalRadiusX) isColliding = true;
